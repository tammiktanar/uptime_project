...
const { getAllUsers, getUserWithEmail } = require('../mysql/get/users');
...
router.post('/', async function(req, res, next) {
    ...
    if (req.body.username.length >= 200) {
        res.status(500).json({success: false, error: true, message: "Username too long", data: req.body})
        return
    }

    if (req.body.email.length >= 200) {
        res.status(500).json({success: false, error: true, message: "Email too long", data: req.body})
        return
    }
